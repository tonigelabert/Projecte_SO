...
int main(int argc, char *argv[]) {
	...
	serv_adr.sin_port = htons(9050); //9070 en que puerto vamos a escuchar
...
void Consulta_RetornarContrasenya(MYSQL *conn, char buff2[512], char nom[20], char paraulaSeguretat[20])
{
	...
	resultado = mysql_store_result (conn);
	row = mysql_fetch_row (resultado);
	//Només hi ha un resultat possible per la consulta
	buff2[0] = NULL;
...
void Registrar(MYSQL *conn, char buff2[20], char* nom[20], char contrasenya[20], char paraulaSeguretat[20])
{
	int err;
	// Estructura especial para almacenar resultados de consultas 
	MYSQL_RES *resultado;
